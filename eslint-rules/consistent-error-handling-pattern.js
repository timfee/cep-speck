module.exports = {
  ...
  create(context) {
    ...
    function getFileContext(fileName) {
      if (fileName.includes("/spec/items/") || 
          fileName.includes("/spec/validate") ||
          fileName.includes("/spec/healing/")) {
        return "validation";
      }
      if (fileName.includes("/spec/streaming") || 
    ...
    return {
      NewExpression(node) {
        if (node.callee.name === "Error") {
          ...
          if (fileContext === "validation") {
            context.report({
              node,
              message: "Use Issue interface for validation errors, not Error class",
            });
      ...
      ThrowStatement(node) {
        ...
        if (fileContext === "validation") {
          context.report({
            node,
            message: "Validation modules should return Issue[] from validate(), not throw errors",
          });
        }
        
        if (fileContext === "streaming" && 
            node.argument && 
            node.argument.type === "NewExpression" &&
            node.argument.callee.name === "Error") {
          context.report({
            node,
            message: "Throw StreamingError instances in streaming context, not generic Error",
          });
        }
      },
      
      Identifier(node) {
        if (node.name === "Issue" || node.name === "ErrorDetails" || node.name === "StreamingError") {
          const fileContext = getFileContext(fileName);
          ...
          if (node.name === "StreamingError" && fileContext !== "streaming") {
            context.report({
              node,
              message: "StreamingError should only be used in streaming/API context",
            });
          }
          ...
          if (node.name === "ErrorDetails" && fileContext !== "component") {
            context.report({
              node,
              message: "ErrorDetails should only be used in component/UI context",
            });
          }
        }
      },
      ...
      ObjectExpression(node) {
        const hasCode = node.properties.some(p => p.key && p.key.name === "code");
        const hasMessage = node.properties.some(p => p.key && p.key.name === "message");
        const hasRecoverable = node.properties.some(p => p.key && p.key.name === "recoverable");
