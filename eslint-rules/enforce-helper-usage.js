module.exports = {
  ...
  create(context) {
    const helperPatterns = {
      extractSection: /function\s+extract\w*Section|const\s+extract\w*Section\s*=/,
      extractMetrics: /function\s+extract\w*Metrics|const\s+extract\w*Metrics\s*=/,
      extractBulletPoints: /function\s+extract\w*Bullet|const\s+extract\w*Bullet\s*=/,
      buildHealing: /function\s+build\w*Healing|const\s+build\w*Healing\s*=/,
      createWordBoundaryRegex: /function\s+create\w*Boundary|const\s+create\w*Boundary\s*=/,
      createFlexibleRegex: /function\s+create\w*Regex|const\s+create\w*Regex\s*=/,
      doesMetricReferenceFeature: /function\s+\w*MetricReference|const\s+\w*MetricReference\s*=/,
    };
    ...
    return {
          ...
          if (pattern.test(functionText) && !helperImports.has(helperName)) {
            context.report({
              ...
              fix(fixer) {
                ...
                const firstImport = node.parent.body.find(n => n.type === "ImportDeclaration");
                if (firstImport) {
      ...
      CallExpression(node) {
        ...
        if (callee.type === "MemberExpression") {
          ...
          if (property.name === "match" && node.arguments[0]?.type === "Literal") {
            const regexPattern = node.arguments[0].value;
            
            if (regexPattern && regexPattern.toString().includes("^#\\s+\\d+\\.")) {
              context.report({
                node,
            ...
            if (regexPattern && regexPattern.toString().includes("^[-*]\\s+")) {
              context.report({
                node,
                message: "Use extractBulletPoints helper for bullet point extraction",
              });
            }
          }
        }
      ...
      Literal(node) {
        if (node.regex && typeof node.value === "object") {
          ...
          if (pattern.includes("\\b") && pattern.includes("\\b")) {
            context.report({
              node,
              message: "Use createWordBoundaryRegex helper for word boundary patterns",
            });
          }
          ...
          if (pattern.startsWith("(?i)")) {
            context.report({
              node,
              message: "Use createFlexibleRegex helper for case-insensitive patterns",
            });
          }
        }
      },
