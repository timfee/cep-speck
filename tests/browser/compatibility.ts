    ...
    try {
      for (const frameData of frames) {
        ...
        try {
          ...
          errors.push(
            `Frame parsing error: ${parseError instanceof Error ? parseError.message : String(parseError)}`
          );
        }
      ...
      errors.push(
        `Streaming test error: ${error instanceof Error ? error.message : String(error)}`
      );
    }
