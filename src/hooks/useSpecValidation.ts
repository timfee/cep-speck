...
const VALIDATION_PATTERNS = {
  ...
  TARGET_USERS:
    /^(?:target\s*)?(?:audience|users?|customers?|personas?)\s*[:=]/im,
...
export function useSpecValidation(content: string): SpecValidation {
  return useMemo(() => {
    ...
    if (!isStructuredFormat)
      issues.push("Consider using structured format (key: value)");
    if (words.length < 10) issues.push("Specification seems too brief");
    if (words.length > 200)
      issues.push("Consider using more concise input for better AI generation");
