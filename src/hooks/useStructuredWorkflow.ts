...
const generateContentOutline = (prompt: string): ContentOutline => {
  ...
  if (
    lowercasePrompt.includes("enterprise") ||
    lowercasePrompt.includes("security") ||
    lowercasePrompt.includes("admin")
  ) {
    functionalRequirements.push({
      id: "fr-auth-security",
  ...
  if (
    lowercasePrompt.includes("integration") ||
    lowercasePrompt.includes("api") ||
    lowercasePrompt.includes("connect")
  ) {
    functionalRequirements.push({
      id: "fr-integrations",
  ...
  const progress = useMemo((): WorkflowProgress => {
    ...
    switch (state.currentStep) {
      case "idea":
        ...
        completion = Math.min(
          100,
          (state.initialPrompt.length / PROMPT_COMPLETION_TARGET) * 100
        );
        break;
      case "outline":
        const totalItems =
