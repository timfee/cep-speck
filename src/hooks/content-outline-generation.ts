...
export async function generateContentOutline(
  ...
  try {
    ...
    return parseContentOutlineResponse(fullResponse);
  } catch (error) {
    ...
    if (contextKeywords.includes('dlp') || contextKeywords.includes('data loss')) {
      functionalRequirements.push({
        ...
        description: "Intelligent nudge system that prompts admins to enable DLP rules when they access the DLP page with zero configured rules",
        priority: "P0",
        userStory: "As a Chrome admin, I want to be guided to enable DLP protection when I have no rules configured, so that my organization's data is protected",
        acceptanceCriteria: [
      ...
      functionalRequirements.push({
        ...
        description: "Default audit-only DLP rule configuration that can be quickly enabled by admins",
        priority: "P0",
        userStory: "As a Chrome admin, I want to quickly enable a safe audit-only DLP rule to start monitoring data flows",
        acceptanceCriteria: [
      ...
      functionalRequirements.push({
        ...
        description: "Context-aware onboarding that guides users through setup based on their current configuration state",
        priority: "P0",
        userStory: "As an admin, I want to be guided through setup when I haven't configured key features",
        acceptanceCriteria: [
      ...
      functionalRequirements.push({
        ...
        description: prompt.length > 50 ? "Primary functionality based on the described requirements" : "Primary product capabilities based on the described use case", // eslint-disable-line no-magic-numbers
        priority: "P0",
        userStory: "As a user, I want to accomplish the main objectives described in the requirements",
        acceptanceCriteria: ["Feature works as described", "User can complete primary workflows", "Performance meets expectations"],
      });
    }
    ...
    if (contextKeywords.includes('dlp') || contextKeywords.includes('admin')) {
      successMetrics.push({
        ...
        description: "Percentage of admins who enable DLP rules after seeing the nudge",
        type: "adoption",
    ...
    if (contextKeywords.includes('dlp') || contextKeywords.includes('nudge')) {
      milestones.push({
        ...
        description: "Research admin workflows and analyze existing DLP rule patterns",
        phase: "research",
        estimatedDate: "Month 1",
        deliverables: ["Admin workflow analysis", "DLP rule usage patterns", "Nudge design requirements"],
      });
      ...
      milestones.push({
        ...
        deliverables: ["Interactive prototype", "Admin feedback", "Refined nudge design"],
      });
      ...
      milestones.push({
        ...
        deliverables: ["Working nudge system", "Pre-configured rules", "Admin testing"],
      });
    } else {
      milestones.push({
        id: "ms-development",
