...
import type { EvaluationIssue, StructuredOutline } from "@/lib/agents/types";
...
export const useAgenticWorkflow = () => {
  ...
  const setError = useCallback((error: string | null) => {
    setState((prev) => ({
      ...
      error,
      isLoading: false,
      phase: error !== null ? "error" : prev.phase,
    }));
  }, []);
   ...
   */
  const evaluateDraftInternal = useCallback(
    async (draftToEvaluate: string) => {
      ...
      try {
        ...
        setError(
          error instanceof Error ? error.message : "Failed to evaluate draft"
        );
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
