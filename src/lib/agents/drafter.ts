 ...
 */
export async function runDrafterAgent(outline: StructuredOutline): Promise<StreamTextResult<Record<string, never>, never>> {
  try {
    ...
    const needsCompetitorResearch = /competitor|competition|market/i.test(outlineText);
    ...
    const messages: CoreMessage[] = [
      ...
      { role: 'user', content: `Generate a complete PRD based on this outline:\n\n${JSON.stringify(outline, null, 2)}` },
    ];
...
function extractCompetitorMentions(outline: StructuredOutline): string[] {
  ...
  for (const section of outline.sections) {
    if (section.notes) {
      ...
      const competitorPattern = /(?:check|research|analyze)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi;
