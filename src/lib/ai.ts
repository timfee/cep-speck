...
function createGeminiModel(config: AIConfig = {}) {
  ...
  if (apiKey === undefined || apiKey === "") {
    throw new Error(
      "No API key available. Please configure your Google AI API key."
    );
  }
...
export async function generatePRDContent(
  ...
  const schema = z.object({
    ...
    overview: z
      .string()
      .describe(
        "Comprehensive product overview explaining the purpose and value"
      ),
    objectives: z
      .array(z.string())
      .describe(
        "Key business and user objectives this product aims to achieve"
      ),
    requirements: z
      .array(
        z.object({
          ...
          description: z
            .string()
            .describe(
              "Detailed requirement description with specific criteria"
            ),
          priority: z
            .enum(["high", "medium", "low"])
            .describe("Priority level based on business impact"),
        })
      )
    ...
    success_metrics: z
      .array(z.string())
      .describe("Measurable success criteria and KPIs"),
  });
...
Provide actionable, detailed content that development teams can implement.`;
    ...
    onProgress?.(
      "thinking",
      "AI analyzing requirements and generating structured PRD..."
    );
      ...
      .array(
        z.object({
          ...
          target: z
            .string()
            .describe(
              "Specific, measurable target with baseline and timeframe"
            ),
        })
      )
