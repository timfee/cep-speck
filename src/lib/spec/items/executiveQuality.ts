...
export function validate(draft: string, params: Params): Issue[] {
  ...
  if (params.enforceFactualTone) {
    const vagueQuantifiers = draft.match(/\b(many|several|numerous|various|multiple|some|most|few)\s+(?!days|weeks|months|years|minutes|seconds|hours)\w+/gi);
    if (vagueQuantifiers && vagueQuantifiers.length > 3) {
      issues.push({
        id: 'vague-quantifiers',
        itemId,
        severity: 'warn',
        message: 'Too many vague quantifiers - use specific numbers where possible',
        evidence: `Found ${vagueQuantifiers.length} instances: ${vagueQuantifiers.slice(0, 3).join(', ')}...`
      });
    }
    
    const hedgingLanguage = draft.match(/\b(potentially|possibly|might|could|may|perhaps|likely|probably)\s+\w+/gi);
    ...
    const inventedMetrics = draft.match(/\b(NPS|Net Promoter Score|satisfaction score|happiness index|engagement score)\b/gi);
    if (inventedMetrics && inventedMetrics.length > 0) {
      issues.push({
        id: 'quality-theater-metrics',
        itemId,
        severity: 'warn',
        message: 'Avoid quality theater metrics like NPS - focus on operational metrics',
        evidence: `Quality indicators found: ${inventedMetrics.join(', ')}`
      });
    }
    
    const solidifyFuture = draft.match(/\b(solidify our future|strengthen our position|position ourselves|future-proof)\b/gi);
  ...
  if (params.requireSpecificMetrics) {
    ...
    if (metricSections) {
      ...
      const heuristicMasking = draft.match(/\b(because|due to|in order to|to ensure|for the purpose of)\s+[^.]*\b(metric|measure|track|monitor)\b/gi);
      if (heuristicMasking && heuristicMasking.length > 0) {
        issues.push({
          id: 'heuristic-masking',
          itemId,
          severity: 'warn',
          message: 'Consider stating the heuristic directly rather than obscuring through metrics',
          evidence: `Potential heuristic masking: ${heuristicMasking.slice(0, 2).join('; ')}`
        });
      }
    }
  }
    ...
    for (const pattern of overExplanationPatterns) {
      ...
      if (matches && matches.length > 2) {
        issues.push({
          ...
          itemId,
          severity: 'warn',
          message: 'Over-explanation detected - state facts directly without meta-commentary',
          evidence: `Found ${matches.length} instances: ${matches.slice(0, 2).join(', ')}`
        });
      }
    }
  }
...
export function heal(issues: Issue[]): string | null {
  ...
  if (issues.some(i => i.id === 'vague-quantifiers')) {
    healingInstructions.push('Replace vague quantifiers with specific numbers, percentages, or ranges');
  }
  
  if (issues.some(i => i.id === 'excessive-hedging')) {
    healingInstructions.push('State facts directly; use hedging only for genuine uncertainty');
  }
  
  if (issues.some(i => i.id === 'metrics-missing-units')) {
    healingInstructions.push('Add units and timeframes to all metrics (%, ms, minutes, # users, etc.)');
  }
  
  if (issues.some(i => i.id === 'over-explanation')) {
    healingInstructions.push('Remove meta-commentary and explanatory phrases; state facts directly');
  }
