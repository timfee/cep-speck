...
function validate(draft: string, params: Params, _pack?: unknown): Issue[] {
  ...
  for (const line of metricLines) {
    ...
    if (params.require.includes("timeframe") && !/\b(within|by)\b/i.test(line))
      missing.push("timeframe");
    if (
      params.require.includes("units") &&
      !/%|\bms\b|\bminutes?\b|\bdays?\b|\bcount\b/.test(line)
    )
      missing.push("units");
    if (
      params.require.includes("SoT") &&
      !/\b(SoT|Source of Truth|CBCM|Reporting Connector|Admin Console)\b/.test(
        line
      )
    )
      missing.push("SoT");
